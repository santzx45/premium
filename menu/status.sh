...
ohq=$(systemctl status ovpn-ohp | grep -i "active (running)")
...
echo -e " $mg - $off $bd OHP Dropbear      $off    : $sohp "
echo -e " $mg - $off $bd OHP OpenVpn          $off : $sohq "
echo -e " $mg - $off $bd OHP SSH            $off   : $sohr "
...
sec=10
         ...
         sleep 1
                ...
                systemctl restart openvpn
                systemctl restart dropbear
                systemctl start dropbear-ohp
                systemctl start ovpn-ohp
                systemctl start ssh-ohp
fi
echo ""
echo -e "${cyan}================================================${off}"
echo -e ""
